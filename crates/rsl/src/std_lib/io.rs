...
pub fn agent_read_file(arguments: Vec<Primitive>) -> Primitive {
    if let (Some(Primitive::String(workspace_dir)), Some(Primitive::String(path))) =
...
pub fn agent_write_file(arguments: Vec<Primitive>) -> Primitive {
    if let (
        Some(Primitive::String(workspace_dir)),
        Some(Primitive::String(file_path)),
        Some(Primitive::String(content)),
...
pub fn agent_replace(arguments: Vec<Primitive>) -> Primitive {
    if let (
        Some(Primitive::String(workspace_dir)),
        Some(Primitive::String(file_path)),
        Some(Primitive::String(old_string)),
        Some(Primitive::String(new_string)),
    ) = (
        arguments.first(),
        arguments.get(1),
        arguments.get(2),
...
pub fn run_shell_command(arguments: Vec<Primitive>) -> Primitive {
    if let (Some(Primitive::String(command)), Some(Primitive::String(workspace_dir))) =
        (arguments.first(), arguments.get(1))
    {
    ...
    Primitive::Error("Expected file path".to_string())
}
