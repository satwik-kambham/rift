...
impl Decoder {
    ...
    fn decode(&mut self, mel: &Tensor, t: f64) -> DecodingResult {
    ...
    fn decode_with_fallback(&mut self, segment: &Tensor) -> DecodingResult {
    ...
    fn run(&mut self, mel: &Tensor) -> Vec<Segment> {
...
fn main() {
    ...
    let recording = "/home/satwik/Documents/rift/examples/android/audio_1746629457596.wav";
    let config = "/home/satwik/Documents/rift/examples/whisper/tiny-en/config.json";
    let model = "/home/satwik/Documents/rift/examples/whisper/tiny-en/model.safetensors";
    let tokenizer = "/home/satwik/Documents/rift/examples/whisper/tiny-en/tokenizer.json";

    let device = Device::Cpu;

    let input = std::path::PathBuf::from(recording);
    ...
    println!("Loaded tokenizer and config: {}s", start.elapsed().as_millis() as f32 / 1000.0);
    ...
    println!("Loaded melfilers: {}s", start.elapsed().as_millis() as f32 / 1000.0);
    ...
    .unwrap();
    println!("Loaded and preprocessed input: {}s", start.elapsed().as_millis() as f32 / 1000.0);

    let vb = unsafe { VarBuilder::from_mmaped_safetensors(&[model], m::DTYPE, &device).unwrap() };
    let model = m::model::Whisper::load(&vb, config).unwrap();
    println!("Loaded model: {}s", start.elapsed().as_millis() as f32 / 1000.0);

    let mut decoder = Decoder::new(model, tokenizer, 299792458, &device, None, false);
    decoder.run(&mel);
    println!("Transcribed audio: {}s", start.elapsed().as_millis() as f32 / 1000.0);
}
